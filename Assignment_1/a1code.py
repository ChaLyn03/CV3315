...
def print_stats(image):
    ...
    print(f"Image dimensions: {h} x {w}, Channels: {channels}")
...
def change_contrast(image, factor):
    """
    Adjust contrast using the formula: new_pixel = factor*(old_pixel - 0.5) + 0.5.
    Pixel values are clipped to remain in the [0,1] range.
    """
    new_image = factor * (image - 0.5) + 0.5
    # Clip to ensure values remain within [0, 1]
    return np.clip(new_image, 0, 1)


# --- Process Only One Channel of an RGB Image ---
# For example, apply the contrast adjustment only to the red channel (channel index 0)

def change_contrast_channel(image, channel, factor):
    """
    Apply contrast change only to a specific channel of an RGB image.
    """
    # Make a copy so we don't modify the original image
    new_image = image.copy()
    new_image[..., channel] = factor * (new_image[..., channel] - 0.5) + 0.5
    new_image[..., channel] = np.clip(new_image[..., channel], 0, 1)
    return new_image
